...
#define DBG_SENT_LEN 8
#define DBG_RECV_LEN 32   /* enough for one Claude Code redraw round-trip */
static unsigned char dbg_sent[DBG_SENT_LEN];
static int dbg_sent_count = 0;
static unsigned char dbg_recv[DBG_RECV_LEN];
static int dbg_recv_count = 0;

static void dbg_log_sent(const char *buf, int n) {
    for (int i = 0; i < n; i++) {
        dbg_sent[dbg_sent_count % DBG_SENT_LEN] = (unsigned char)buf[i];
        dbg_sent_count++;
    }
}
...
static void dbg_log_recv(const char *buf, int n) {
    for (int i = 0; i < n; i++) {
        dbg_recv[dbg_recv_count % DBG_RECV_LEN] = (unsigned char)buf[i];
        dbg_recv_count++;
    }
}

/* Format a hex string of the most recent ring_len bytes, oldest-first.
 * Caller passes the ring buffer, its capacity, and total bytes ever
 * written. */
static void dbg_fmt(char *out, int out_sz,
                    const unsigned char *ring, int ring_len, int total) {
    int n = total < ring_len ? total : ring_len;
    int start = (total - n + ring_len) % ring_len;
    if (total < ring_len) start = 0;
    int p = 0;
...
static void draw_status(renderer_t *r, const ssh_config_t *cfg,
    ...
    renderer_draw_text(r, 1, 3, line,
                       (kbd && kbd->sticky_ctrl == MOD_ARMED)
                           ? COLOR_ACCENT : COLOR_DIM);

    /* Diagnostic readout — eats most of the panel because debugging
     * Claude Code rendering is the active concern.  When this milestone
     * is finished we'll trim back. */
    renderer_draw_text(r, 1, 5, "DEBUG", COLOR_ACCENT);
    if (term) {
        snprintf(line, sizeof(line),
                 "cur=(%d,%d) m=%d/%d alt=%d",
                 term->cur_x, term->cur_y,
                 term->mouse_proto, term->mouse_sgr, term->use_alt);
        renderer_draw_text(r, 1, 6, line, COLOR_DIM);
    }
    char hex[96];
    dbg_fmt(hex, sizeof(hex), dbg_sent, DBG_SENT_LEN, dbg_sent_count);
    snprintf(line, sizeof(line), "sent: %s", hex);
    renderer_draw_text(r, 1, 7, line, COLOR_DIM);

    /* Recv: split 32 bytes across 4 lines of 8 = 24 hex chars per line. */
    char rhex[120];
    dbg_fmt(rhex, sizeof(rhex), dbg_recv, DBG_RECV_LEN, dbg_recv_count);
    int rhex_len = (int)strlen(rhex);
    char chunk[28];
    for (int k = 0; k < 4; k++) {
        int start = k * 24;
        if (start >= rhex_len) break;
        int n = (start + 24 > rhex_len) ? (rhex_len - start) : 24;
        memcpy(chunk, rhex + start, n);
        chunk[n] = 0;
        snprintf(line, sizeof(line), k == 0 ? "recv: %s" : "      %s", chunk);
        renderer_draw_text(r, 1, 8 + k, line, COLOR_DIM);
    }

    renderer_draw_text(r, 1, 14, "KEY LEGEND", COLOR_ACCENT);
    renderer_draw_text(r, 1, 15, "  A=Enter  B=BS  X=type  D-pad arrows", COLOR_FG);
    renderer_draw_text(r, 1, 16, "  L+key=Ctrl-key  SELECT=arm Ctrl  STRT=quit", COLOR_FG);
    renderer_draw_text(r, 1, 17, "  L+Y emergency: send 'q' (exit tmux copy)", COLOR_FG);

    renderer_draw_text(r, 1, 19, "Press B in claude code, photograph DEBUG", COLOR_DIM);
}
